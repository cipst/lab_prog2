...
public class Cons extends List {
    ...
    public List append(List l) {
        ...
        /**
         * NOTA SULL'EFFICIENZA DI append(). Il nostro append() richiede un numero di
         * usi di new uguali a: (1/2) * this.size() * l.size() (circa) Ci sono versioni
         * di append() molto piu' veloci, che richiedono meno usi di new, all'incirca
         * (this.size() + l.size()) Pero' sono piu' difficili: per curiosita', qui sotto
         * includiamo un algoritmo detto di MERGING: fonde due liste ordinate in una
         * lista ordinata. if (l.empty()) return this; else if (this.empty()) return l;
         * else { int x = ((Cons) l).elem; List m = ((Cons) l).next; if (x< elem) return
         * new Cons(x,append(m)); else if (x == elem) return new Cons(x,next.append(m));
         * else return new Cons(elem,next.append(l)); }
         */
    }
    ...
    public int get(int i) {
        assert i >= 0 : "Impossibile accedere all'elemento in pos " + i;
        if (i > 0)
    ...
    public List intersect(List l) {
        if (l.contains(this.elem))
            return new Cons(this.elem, next.intersect(l));
        else
